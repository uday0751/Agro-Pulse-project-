...
export default function CustomerMarketplacePage() {
  ...
  return (
    <div className="min-h-screen w-full px-4 sm:px-6 md:px-8 lg:px-10 py-6 pt-[84px] font-sans">
      ...
      {/* BROWSE CROPS TO BUY (TRUE 100% FULL WIDTH FLUID STRETCH GRID) */}
      {activeTab === "browse" && (
        <div className="space-y-8 w-full">
          ...
          {/* LISTINGS DISPLAY (FLEX GROW TO FILL 100% OF FULL SCREEN WIDTH) */}
          {filteredListings.length === 0 ? (
            ...
            Object.entries(groupedData).map(([groupTitle, items]) => (
              <div key={groupTitle} className="mb-10 w-full">
                ...
                {/* FLEX FLUID CONTAINER: CARDS GROW TO FILL 100% PAGE WIDTH ALWAYS! */}
                <div className="flex flex-wrap gap-6 md:gap-8 w-full">
                  {items.map((listing) => (
                    <motion.div
                      ...
                      className="flex-1 min-w-[300px] md:min-w-[340px] max-w-full bg-white dark:bg-[#1a1b23] border-2 border-gray-100 dark:border-white/10 hover:border-emerald-500 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all flex flex-col justify-between group"
                    >
                      <div>
                        {/* Image Header with Click to Inspect Overlay */}
                        <div 
                          onClick={() => setInspectingCrop(listing)}
                          className="relative h-60 w-full bg-gray-100 dark:bg-white/5 overflow-hidden cursor-pointer"
                        >
                          <img 
                          ...
                          />
                          <span className="absolute top-3.5 left-3.5 bg-emerald-600 text-white text-[11px] font-extrabold px-3 py-1 rounded-xl shadow-md flex items-center gap-1">
                            <ShieldCheck className="w-3.5 h-3.5" /> {listing.qualityGrade}
                          </span>

                          <span className="absolute bottom-3.5 right-3.5 bg-black/75 backdrop-blur-md text-white text-xs font-extrabold px-3 py-1.5 rounded-xl flex items-center gap-1.5">
                            <Truck className="w-4 h-4 text-emerald-400" /> {listing.deliveryOption}
                          </span>

                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <span className="bg-white/90 text-gray-900 font-extrabold px-4 py-2.5 rounded-2xl text-xs flex items-center gap-2 shadow-lg">
                              <Eye className="w-4 h-4 text-emerald-600" /> View Produce Details
                            </span>
                          </div>
                        </div>

                        {/* Card Details */}
                        <div className="p-6" onClick={() => setInspectingCrop(listing)}>
                          <div className="flex justify-between items-start mb-3 cursor-pointer">
                            <div>
                              <h3 className="font-black text-gray-900 dark:text-white text-lg leading-snug group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                {listing.cropName}
                              </h3>
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="flex items-center text-xs text-yellow-500 font-bold bg-yellow-50 dark:bg-yellow-950/30 px-2 py-0.5 rounded-md">
                                  <Star className="w-3.5 h-3.5 fill-yellow-400 mr-1" /> {listing.rating}
                                </span>
                              </div>
                            </div>
                            <span className="text-3xl">{listing.iconEmoji}</span>
                          </div>
                          ...
                          {/* Price Banner */}
                          <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 p-4 rounded-2xl flex justify-between items-center mb-1">
                            <div>
                              <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">APMC Mandi Rate</span>
                              <div className="text-xl font-black text-emerald-700 dark:text-emerald-400">₹{listing.pricePerQuintal.toLocaleString("en-IN")}<span className="text-xs font-bold text-gray-500">/quintal</span></div>
                            </div>
                            <div className="text-right">
                              <span className="text-[10px] text-gray-400 block font-bold uppercase tracking-wider">Available Stock</span>
                              <span className="text-sm font-black text-gray-900 dark:text-white">{listing.availableQuantityQuintals} Quintals</span>
                            </div>
                          </div>
                        </div>
                      </div>
